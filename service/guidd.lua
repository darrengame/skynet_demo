...
local function update_generator(idtype, update_cache)
    ...
    if not update_cache then
        ...
        if cnt < step/10 then
            if not init_generator(idtype, step) then
                log.debug("can't get new id. idtype:", idtype)
            end
        end
    end
    ...
    if old_block ~= info.cur_idx then
        log.debug("switch id block. idtype:", idtype, ", cur:", info.cur_idx, ", old:", old_block, ", ids_cnt:", cnt)
    end
end
...
local function get_new_id(idtype)
    ...
    if not info.cur_idx then
        if info.ids_cnt > 0 then
            ...
            if not info.cur_idx then
                log.debug("new guid too busy. idtype:", idtype)
                return
            end
        else
            log.debug("id pool null. idtype:", idtype)
            return
        end
    end
    ...
    if diff <= 0 then
        -- 本段已经消耗完，正在切段
        log.debug("id block all used. idtype:", idtype)
        return
    end

    local new_id = diff + cur_idx
    if diff == 1 then
        log.debug("id block used. cur:", cur_idx)
...
local function create_new_ids()
    while true do
        for idtype, info in pairs(block_datas) do
            log.debug("create going to check id space. idtype:", idtype)
            if info.ids_cnt < info.step/10 then
                log.debug("creator start update id space. idtype:", idtype)
                update_generator(idtype)
                log.debug("creator update id space ok. idtype:", idtype)
            else
                log.debug("not need create new ids. idtype:", idtype)
            end
        end
        is_busy = false
        skynet.wait()
